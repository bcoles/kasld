...
**guaranteed** window (the one resolved at the sound floor).

`source` is the kernel: `alpine` (a distro kernel) or `mainline` (a vanilla
kernel.org build via `tests/vm/build-kernel`). `residual` says how much KASLR
entropy KASLD could *not* strip from the base — i.e. how effectively KASLR was
defeated:
...
- `—` — KASLR is off, so there is no randomization to defeat.

The axis is architecture × reader profile; a kernel-config axis is out of scope
(see Scope below).

| arch | release | source | scenario | KASLR | residual |
|------|---------|--------|----------|-------|----------|
| x86_64 | 6.12.81-0-virt | alpine | default | on | exact |
| x86_64 | 6.12.81-0-virt | alpine | hidden | on | 1 bit |
| x86_64 | 6.12.81-0-virt | alpine | hardened | on | 6 bits |
...
| aarch64 | 6.12.81-0-virt | alpine | default | on | exact |
| aarch64 | 6.12.81-0-virt | alpine | hidden | on | 10 bits |
| aarch64 | 6.12.81-0-virt | alpine | hardened | on | 26 bits |
...
| s390x | 6.12.81-0-lts | alpine | default | on | exact |
| s390x | 6.12.81-0-lts | alpine | hidden | on | 22 bits |
| s390x | 6.12.81-0-lts | alpine | hardened | on | 39 bits |
| mips | 6.15.6 | mainline | default | on | exact |
| mips | 6.15.6 | mainline | hidden | on | exact |
| mips | 6.15.6 | mainline | hardened | on | 10 bits |
| mipsel | 6.15.6 | mainline | default | on | exact |
| mipsel | 6.15.6 | mainline | hidden | on | exact |
| mipsel | 6.15.6 | mainline | hardened | on | 10 bits |
...
| ppc32 | 6.15.6 | mainline | hardened | on | 11 bits |
| x86_64 | 6.15.6 | mainline | default | on | exact |
| x86_64 | 6.15.6 | mainline | hidden | on | 5 bits |
| x86_64 | 6.15.6 | mainline | hardened | on | 9 bits |
