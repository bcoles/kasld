...
These techniques are KASLD's *leak* inputs — surfaces that disclose an address.
They are one of two input kinds the tool consumes: the other is non-disclosing
*facts and constraints* (kernel config, `MemTotal`, image size, VA-bits probes,
cmdline memory limits, no-seed/nokaslr detection) that bound the search space
without revealing an address. Those, and the inference engine that combines both
kinds into a resolved layout, are described in
[architecture.md](architecture.md#the-inference-engine). This document catalogues
the leaks, not KASLD's full component set.
...
- [Weak entropy](#weak-entropy)
  - [Randomization failure at boot](#randomization-failure-at-boot)
  - [Slide baseline](#slide-baseline)
- [Patched kernel bugs](#patched-kernel-bugs)
- [Exploit primitives](#exploit-primitives)
  - [Leaking a pointer from a reachable object](#leaking-a-pointer-from-a-reachable-object)
  - [Reading a fixed, KASLR-invariant structure](#reading-a-fixed-kaslr-invariant-structure)
  - [Resolving symbols from an in-kernel vantage](#resolving-symbols-from-an-in-kernel-vantage)
  - [Corruption without a leak](#corruption-without-a-leak)
  - [Probing candidate bases](#probing-candidate-bases)
- [See also](#see-also)
...
* [proc_iomem_kernel.c](../src/components/proc_iomem_kernel.c) — the kernel image's physical extents from the `Kernel code`/`data`/`bss` lines of `/proc/iomem` (`Kernel code` starts at `__pa_symbol(_text)`, the image base); the addresses are masked to `0` without `CAP_SYS_ADMIN`, independent of `kptr_restrict`
...
| Attack | Year | In KASLD | Status | References |
|---|---|---|---|---|
| KernelSnitch | 2025 | experimental | **Implemented (experimental)**: [kernelsnitch.c](../src/components/kernelsnitch.c)<br>Futex hash-table timing leaks `mm_struct` directmap address (not `_stext`). x86_64, unprivileged. Requires `--experimental` (~1–30 min runtime). Nominally targeted by `CONFIG_FUTEX_PRIVATE_HASH` (v6.14+), but the unprivileged opt-out prctl `PR_FUTEX_HASH_SET_SLOTS=0` pins the process back onto the global `mm_struct`-keyed hash and defeats it, so the leak remains live; upstream status unfixed. | [KernelSnitch: Side-Channel Attacks on Kernel Data Structures](https://lukasmaar.github.io/papers/ndss25-kernelsnitch.pdf) (Maar et al., 2025) — [NDSS 2025](https://www.ndss-symposium.org/ndss-paper/kernelsnitch-side-channel-attacks-on-kernel-data-structures/)<br>[lukasmaar/kernelsnitch](https://github.com/lukasmaar/kernelsnitch) |
| Demote+Time (`CLDEMOTE`) | 2025 | — | Intel CPUs implementing the `CLDEMOTE` ISA extension, introduced with the 4th-generation Xeon Scalable (Sapphire Rapids) and the low-power Atom line. The instruction is unprivileged, moves a cache line from an upper-level cache to the LLC, and suppresses faults, so timing repeated demotes of a candidate address distinguishes slots the kernel occupies from slots it does not. Located the kernel base in 2.49 ms at 99.8 % accuracy over 1000 runs on a Xeon Silver 4510T. Where KPTI is active the mapped trampoline region is what remains visible, as for the other page-presence oracles. The authors propose countermeasures — disabling the extension, or backing invalid kernel addresses with dummy pages — but describe none as deployed. | [Demoting Security via Exploitation of Cache Demote Operation in Intel's Latest ISA Extension](https://arxiv.org/pdf/2503.10074) (Kim, Jang, Shin, 2025) |
| Branch Privilege Injection (CVE-2024-45332) | 2025 | — | Intel CPUs with eIBRS, 9th-generation Coffee Lake Refresh through Raptor Lake Refresh. A race between branch resolution and privilege-domain feedback lets user-mode training steer kernel speculation. Derandomizes the kernel image base through BTB collisions in 15 ms at 98.4 % accuracy, and the `physmap` base in 102 ms at 98.2 %. Mitigated by Intel microcode (`IPRED_DIS_S`); AMD and ARM processors showed no sign of the underlying predictor race. | [Branch Privilege Injection: Compromising Spectre v2 Hardware Mitigations by Exploiting Branch Predictor Race Conditions](https://comsec.ethz.ch/wp-content/files/bprc_sec25.pdf) (Rüegge, Wikner, Razavi, 2025) |
...
| Whisper / TET-KASLR (transient-execution timing) | 2024 | — | Intel x86_64. Extends the transient-execution *timing* channel above: the wall-clock duration of a transient sequence, read by ordinary architectural timing rather than a cache covert channel, distinguishes microarchitectural conditions. The authors report a KASLR break that succeeds **with KPTI and/or FLARE deployed** — the mitigations that stop the 2023 variant and EchoLoad — and reimplement Meltdown, ZombieLoad and Spectre-RSB over the same channel. As a transient-execution technique it remains bounded by the in-silicon fixes that close those attacks on recent parts; the precise per-CPU preconditions are the paper's. | [Whisper: Timing the Transient Execution to Leak Secrets and Break KASLR](https://dl.acm.org/doi/10.1145/3649329.3656213) (Jin et al., 2024) — DAC '24, [Kingfish404/whisper-tet](https://github.com/Kingfish404/whisper-tet) |
...
| CounterLeak / Meltdown 3a (CVE-2018-3640) | 2023 | — | Transiently reads privileged system registers from unprivileged code. Where `rdpmc` leaks, a counter driven by page-table walks (`DTLB_LOAD_MISSES.WALK_COMPLETED_2M_4M`) reveals which kernel pages are present, locating `__start_rodata` in 4.7 s at 98 % accuracy. A 19-CPU survey found which register leaks varies per model, and the `rdpmc` leak appears only on Intel Atom-family parts of 2016–2017 (Cherry Trail, Apollo Lake, Gemini Lake); Alder Lake and later show no leakage, and the AMD parts that leak at all leak `rdfsbase`/`rdgsbase`, which carry no kernel address. The attack further presumes the target counter is already programmed. | [Reviving Meltdown 3a](https://misc0110.net/files/meltdown3a_esorics23.pdf) (Weber et al., 2023) — [ESORICS 2023](https://link.springer.com/chapter/10.1007/978-3-031-51479-1_5) |
| Zenbleed (CVE-2023-20593) | 2023 | — | AMD Zen 2 only. Mishandling of `vzeroupper` under speculation leaves stale `YMM` register contents readable across processes, containers and VMs at about 30 kb per core per second. Opportunistic sampling with no targeted KASLR primitive — a kernel pointer is recovered only if one happens to transit the register file — and mitigated by AMD microcode or the `DE_CFG[9]` chicken bit. | [Zenbleed](https://lock.cmpxchg8b.com/zenbleed.html) (Tavis Ormandy, 2023)<br>[AMD-SB-7008](https://www.amd.com/en/resources/product-security/bulletin/amd-sb-7008.html) |
| AVX masked load/store timing | 2023 | yes | **Implemented**: [avx_maskload.c](../src/components/avx_maskload.c)<br>x86_64, Intel and AMD. The AVX masked load/store instructions **suppress faults** where the mask bits are clear, so an unprivileged process can present a kernel address to the MMU with no exception, no fault handler and no cache covert channel; the latency then reports how far the translation got. The two vendors present that difference in opposite directions and the component measures which rather than assuming: on an Intel Core i7-12800H the image is a contiguous band of *faster* slots whose left edge is the base, pinned byte-exact; on a Ryzen 5 5600GT kernel addresses walk the tables whether or not they are mapped, so what stands out instead is a handful of *slower* slots where 4 KiB mappings deepen the walk, reported as interior samples of the image. A hypervisor attenuates but does not remove the signal. **KPTI defeats it** on both vendors — it unmaps the image from the page tables the caller walks, leaving nothing to distinguish an image slot from an empty one, so the technique is confined to parts running without KPTI; measured inert on an Intel Core i7-8750H with KPTI active, and the component detects it and declines rather than sweeping. Nothing else is deployed: FG-KASLR does not prevent it and the paper reports bypassing FLARE. Disclosed to Intel and AMD in July 2022. | [AVX Timing Side-Channel Attacks against Address Space Layout Randomization](https://arxiv.org/pdf/2304.07940) (Choi, Kim, Shin, 2023) — 60th DAC<br>[AVXProbe: Enhancing Website Fingerprinting with Side-Channel-Assisted Kernel-Level Traces](https://dl.acm.org/doi/10.1145/3708821.3710819) (Kim, Na, Kim, Shin & Choi, 2025) — AsiaCCS '25; repurposes the same AVX masked-load timing (via AVX-TSCHA) to derandomize and monitor **kernel-module** address spaces (Intel and AMD, unprivileged). Under KPTI (Linux) / KVAS (Windows) it recovers the base via the mapped entry trampoline plus version offsets — the EntryBleed route above, not the image mapped/unmapped channel this row's component uses — and macOS's Double Map, which fully separates those regions, defeats it. |
...
| ThermalBleed / ThermalScope | 2022 / 2024 | — | Thermal side-channels operate at a ms–second timescale, far too slow/noisy for practical KASLR recovery (which needs sub-µs resolution and must complete before a reboot re-randomizes the slide). The 2024 follow-up ThermalScope breaks KASLR only by reading the mapped/unmapped **prefetch** signal *thermally* — ≈3 billion prefetch instructions over 2 MB intervals, ≈8.2 hours — i.e. a far slower proxy for the direct `prefetch` timing channel below, confirming the timescale limitation rather than overcoming it. | [ThermalBleed: A Practical Thermal Side-Channel Attack](https://ieeexplore.ieee.org/stamp/stamp.jsp?arnumber=9727162) (Kim & Shin, 2022)<br>[ThermalScope: A Practical Interrupt Side Channel Attack Based on Thermal Event Interrupts](https://dl.acm.org/doi/10.1145/3649329.3656525) (Zhang et al., 2024) — DAC '24 |
...
| TLB-based KASLR break on ARM | 2022 | yes | **Implemented**: [arm64_tlb_fault_timing.c](../src/components/arm64_tlb_fault_timing.c)<br>arm64. A store to a kernel address from user space faults either way, but the fault costs measurably more when the address is mapped: the translation walk completes and raises a permission fault, where an unmapped address terminates the walk early. Timing faulting stores across candidate addresses locates the lowest mapped address in the vmalloc region, which is the image base. Needs no cycle counter, no cache covert channel and no speculative execution — `clock_gettime()` suffices — which is why it applies where Data Bounce, Fetch Bounce and EchoLoad do not. Measured separation between mapped and unmapped: 21 % on Cortex-A72, 11 % on Cortex-A53; both time a mapped address slower, the opposite direction to the paper's account, so the component takes the sign from a reference probe of the linear map rather than assuming it. **Mitigated by KPTI**, which arm64 forces on whenever KASLR is enabled (`kaslr_requires_kpti()`) unless the CPU implements E0PD, which blocks EL0 access to kernel addresses without a walk for the same purpose; the component detects the absence of signal and reports KPTI as a confirmed mitigation. A refinement of the double-page-fault lineage below, made to work without the cache. | [Breaking KASLR on Mobile Devices without Any Use of Cache Memory](https://dl.acm.org/doi/10.1145/3560834.3563823) (Seddigh et al., 2022) — ASHES 2022<br>[Extended version](https://link.springer.com/article/10.1007/s13389-023-00344-y) — J Cryptogr Eng 14, 281–294 (2024)<br>[millad7/KASARM](https://github.com/millad7/KASARM) |
...
| FlushConflict (Osiris) | 2021 | — | Intel, Ivy Bridge through Comet Lake. A non-temporal load (`MOVNTDQA`) of an inaccessible kernel address perturbs the cache state of unrelated user memory: the probe line survives in the cache only when the kernel address is not physically backed, giving a mapped/unmapped page oracle. Recovers the start of the kernel image, or the entry trampoline where KPTI is active, in 136 ms on average at 96–100 % accuracy idle and 92–99 % under load, and is unaffected by the hardware fixes deployed through 10th generation — it works on Ice Lake and Comet Lake, where EchoLoad does not. Overlaps the prefetch components where that channel is live, but is mechanically independent of it. | [Osiris: Automated Discovery of Microarchitectural Side Channels](https://arxiv.org/pdf/2106.03470) (Weber, Ibrahim, Nemati, Schwarz, Rossow, 2021) — [USENIX Security 2021](https://www.usenix.org/conference/usenixsecurity21/presentation/weber)<br>[cispa/osiris](https://github.com/cispa/osiris) |
| Speculative dereferencing | 2021 | — | A root-cause correction rather than a separate technique: the effect attributed to prefetching since 2016 is caused by speculative dereferencing of user-space registers inside the kernel, not by the prefetch instruction, and KAISER/KPTI does not eliminate the leakage as previously believed. Describes the mechanism the prefetch components rely on, and enables address-translation attacks from more restricted contexts. | [Speculative Dereferencing of Registers: Reviving Foreshadow](https://arxiv.org/pdf/2008.02307) (Schwarzl, Schuster, Schwarz, Gruss, 2021) |
| EchoLoad | 2020 | experimental | **Implemented (experimental)**: [echoload.c](../src/components/echoload.c)<br>Intel x86_64 only; relies on Meltdown zero-return behavior. Supports TSX, speculation, and signal-handler transient modes. No signal on non-vulnerable hardware (AMD, modern Intel with in-silicon Meltdown fix). KPTI does not fully mitigate: under KPTI the leak lands on the entry trampoline (`__entry_text_start`), and `_stext` is recovered only with KPTI off. Requires `--experimental`. | [KASLR: Break It, Fix It, Repeat](https://gruss.cc/files/kaslrbfr.pdf) (Claudio Canella, Michael Schwarz, Martin Haubenwallner, 2020)<br>[Store-to-Leak Forwarding: There and Back Again](https://i.blackhat.com/asia-20/Friday/asia-20-Canella-Store-To-Leak-Forwarding-There-And-Back-Again-wp.pdf) (Canella et al., 2020) — [Slides](https://misc0110.net/files/store2leak_blackhat_slides.pdf), [Blackhat Asia 2020](https://www.youtube.com/watch?v=Yc1AXkCu2AA)<br>[cc0x1f/store-to-leak-forwarding/echoload](https://github.com/cc0x1f/store-to-leak-forwarding-there-and-back-again/tree/master/echoload) |
...
| Foreshadow-NG / L1TF (CVE-2018-3620) | 2018 | — | Intel. A terminal fault on a not-present page-table entry lets the core speculatively forward L1 data belonging to the frame the stale entry still points at, so an unprivileged process reads kernel memory through disabled page-table entries. Mitigated in mainline since 2018 by PTE inversion — a different mitigation from KPTI, which does not address it. | [Foreshadow-NG](http://foreshadowattack.eu/foreshadow-NG.pdf) (Weisse et al., 2018)<br>[L1TF](https://www.kernel.org/doc/html/latest/admin-guide/hw-vuln/l1tf.html) — kernel documentation |
| Prefetch side-channel | 2016 | yes | **Implemented**: [prefetch.c](../src/components/prefetch.c)<br>Intel and AMD x86_64. Requires KPTI to be disabled (kernel auto-disables KPTI on non-Meltdown-vulnerable CPUs: all AMD, Intel Ice Lake+). Does not require kernel-version-specific offsets. Recent AMD microcode (CVE-2021-26318 / AMD-SB-1017) flattens the kernel-text timing differential on bare metal — a flat result there is the mitigation, not a fault; a batched fallback collector recovers it inside VMs (nested paging reintroduces it) and on unpatched microcode, and where neither pass resolves the edge the component reports no signal rather than a false positive. | [Prefetch Side-Channel Attacks: Bypassing SMAP and Kernel ASLR](https://gruss.cc/files/prefetch.pdf) (Daniel Gruss, Clémentine Maurice, Anders Fogh, 2016)<br>[Using Undocumented CPU Behaviour to See into Kernel Mode and Break KASLR in the Process](https://www.blackhat.com/docs/us-16/materials/us-16-Fogh-Using-Undocumented-CPU-Behaviour-To-See-Into-Kernel-Mode-And-Break-KASLR-In-The-Process.pdf) (Anders Fogh, Daniel Gruss, 2016) — [Blackhat USA](https://www.youtube.com/watch?v=Pwq0vv4X7m4)<br>[xairy/kernel-exploits/prefetch-side-channel](https://github.com/xairy/kernel-exploits/tree/master/prefetch-side-channel)<br>[Fetching the KASLR slide with prefetch](https://googleprojectzero.blogspot.com/2022/12/exploiting-CVE-2022-42703-bringing-back-the-stack-attack.html) (Seth Jenkins, 2022) — [prefetch_poc.zip](https://bugs.chromium.org/p/project-zero/issues/detail?id=2351) |
| Prefetch direct-map (`page_offset_base`) | 2016 | yes | **Implemented**: [prefetch_directmap.c](../src/components/prefetch_directmap.c)<br>The same prefetch primitive applied to the direct map (the linear mapping of all physical RAM), whose base `page_offset_base` is randomized independently of kernel text by `CONFIG_RANDOMIZE_MEMORY` on x86_64. Scans the 1 GiB (PUD)-aligned candidate bases and locates the mapped region's left edge, recovering `page_offset_base` — which resolves the virtual↔physical translation (physical leaks then map to virtual addresses and vice versa). Intel and AMD x86_64; 4-level paging only (the 5-level window spans tens of petabytes, too large for a flat scan, so it declines under `la57`). The 1 GiB-huge-page mapping produces a weaker differential than 2 MiB kernel text, so a run that cannot resolve the edge reports a weak/no signal rather than emitting a false base. | [Prefetch Side-Channel Attacks: Bypassing SMAP and Kernel ASLR](https://gruss.cc/files/prefetch.pdf) (Daniel Gruss, Clémentine Maurice, Anders Fogh, 2016) |
...
* [perf_text_poke_leak.c](../src/components/perf_text_poke_leak.c) — core-kernel `.text` addresses from `PERF_RECORD_TEXT_POKE` side-band records (live-patch, jump-label, ftrace, and kprobe rewrites); the component forces the patching itself by toggling a refcounted static key (requires `kernel.perf_event_paranoid <= 0` or `CAP_PERFMON`)
...
* [alsa_seq_ext_ptr.c](../src/components/alsa_seq_ext_ptr.c) — a kernel direct-map address from an ALSA sequencer variable-length event echoed back through `/dev/snd/seq` (unprivileged with `audio`-group device access; independent of `kptr_restrict`; present in every kernel in git history, fixed in v7.2)
...
This limits the number of possible kernel locations to the values in that table.
...
Beyond the generic `dmesg_kaslr_disabled` classifier, several components detect
an arch-specific no-seed or nokaslr trigger directly, without relying on the
dmesg line:

* [arm64_no_seed.c](../src/components/arm64_no_seed.c) — a non-EFI, device-tree-booted arm64 system with no `/chosen/kaslr-seed` and a CPU lacking `FEAT_RNG` (RNDR): virtual KASLR is off and the kernel sits at `KERNEL_VIRT_TEXT_DEFAULT`
* [riscv64_no_seed.c](../src/components/riscv64_no_seed.c) — a non-EFI riscv64 boot with no `/chosen/kaslr-seed` and a CPU lacking the `zkr` ISA extension: no randomization, kernel at the compile-time default
* [s390_kdump_nokaslr.c](../src/components/s390_kdump_nokaslr.c) — the `elfcorehdr=` cmdline parameter, which forces KASLR off on an s390 kdump crash kernel (detection only; does not leak an address)
* [loongarch_kexec_file_nokaslr.c](../src/components/loongarch_kexec_file_nokaslr.c) — the `kexec_file` cmdline token, which loads the LoongArch kernel at its compile-time default with KASLR off (detection only)
...
`layout.virt_image_base_default`. When KASLR is disabled (opt-out), this
...
* [KASLR in the age of MicroVMs](https://dl.acm.org/doi/10.1145/3492321.3519578) (Holmes, Waterman & Williams, 2022) — microVM monitors such as AWS Firecracker skip the kernel's self-relocation step to cut boot time, which leaves the guest kernel with no KASLR at all; proposes in-monitor KASLR, in which the monitor performs the relocation, to restore randomization cheaply.
* [On Kernel's Safety in the Spectre Era (And KASLR is Formally Dead)](https://dl.acm.org/doi/10.1145/3658644.3670332) (Davoli, Avanzini & Rezk, 2024) — a formal treatment, not a new side-channel: layout randomization is proven a sound defence in the classical model, but no amount of it can restore kernel safety once the attacker has speculative execution and side-channels; safety in the Spectre era must instead come from a separate condition the authors define, which does not rely on randomization at all.
...
kernel address — the leak-free end of data-only exploitation:
  ...
  * **Partial pointer overwrite** — the slide is a multiple of the
    architecture's KASLR granule, so the low bits of every already-slid kernel
    pointer are KASLR-invariant, and overwriting only the low byte(s) of an
    existing pointer retargets it within that granule (a nearby object or
    gadget) with no base knowledge. How many bits are invariant is
    per-architecture, and it bounds how wide the overwrite may be: 21 bits on
    x86_64, x86_32 and riscv64 (2 MiB), 16 on arm64, loongarch64 and MIPS
    (64 KiB), 14 on s390 and ppc32 (16 KiB). A one-byte overwrite stays inside
    the granule everywhere; a two-byte one exactly reaches the arm64, loongarch64
    and MIPS limit, and on s390 and ppc32 already touches bits the slide moves.
...
the aligned candidates — the architecture's KASLR granule sets both the grid and
how many there are, from 2 MiB on x86_64 down to 16 KiB on s390 and ppc32. A
survivable arbitrary read can scan them for the kernel's mapped contents, or —
where nothing can be read back — an observable non-fatal side effect can
distinguish a mapped from an unmapped address. A wrong
...
brute-forced this way, unlike the userland [Brute force](#brute-force) case.
  ...
  * [BlindSide](https://www.vusec.net/projects/blindside/) (Göktaş et al., 2020) — speculative crash suppression removes the single-shot limit: probes that would panic architecturally are executed speculatively, so a memory-write primitive alone derandomizes the kernel, and fine-grained randomization and execute-only text fall with it ([paper](https://download.vusec.net/papers/blindside_ccs20.pdf))
