...
| `check-component-announces` | every component accounts for itself. A blank section under a component header says neither "not applicable here" nor "refused" nor "found nothing", and those are different facts about the target. Two assertions: every source calls `kasld_info` at least once, and every component still writes something when run against a sysroot holding nothing — the second is what catches an announce placed after the early returns, which reads as correct in source and goes silent the moment a source is missing. Live probes skip under a sysroot and are held to the static assertion alone |
...
| `check-shellcheck` | shellcheck over every shell script the tree ships — the operator helpers in `extra/` and the build, lint and replay drivers under `tests/`. The directories are discovered rather than listed, so a new one arrives covered; only the exclusions are named (the fixture corpus, and anything deeper than one level, which keeps the kernel-source cache and the fuzz corpora out) |
...
| `check-diagram-palette` | every committed diagram titles itself in the one house colour. The set was brought to a single palette by editing the SVGs, but three of them are generated and their generators were not touched, so the next regeneration silently restored the old colour — an SVG diff is not read, and a wrong title renders as a perfectly good diagram. The rendered value is asserted rather than the generator, which catches the drift wherever it comes from and needs neither a list of which files have generators nor a way to run them. Both spellings are collected, the `.title` rule and a `fill` on the element, because an inline attribute overrides the rule and a check that stopped at the rule would call a wrongly-rendering file correct. Only the title is pinned: the set uses 58 colours across bars and shading, and a diagram with no title is not a failure |
...
**`check-diagram-data`** — Three of the eighteen diagrams plot data that lives
...
box-drawing glyphs -- covers all eighteen.
