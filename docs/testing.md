...
| `check-target-model` | a capture from an architecture or address width this build does not model is refused, and one it does model is not. Every other fixture harness reads the architecture out of the capture and runs the build for it, so they only ever construct correct pairings; this one prepares each fixture the way an operator does and then points the *native* binary at it, since the wrong pairing is what yields a guaranteed window which cannot contain the base. The expectation is computed per fixture from what the prepared tree states — a kallsyms column of a different width, or the capture record `prepare-bundle` carries in — so a tree stating neither is counted and reported rather than silently passing |
