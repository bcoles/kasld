...
| `check-target-model` | a capture from an architecture or address width this build does not model is refused, and one it does model is not. Every other fixture harness reads the architecture out of the capture and runs the build for it, so they only ever construct correct pairings; this one constructs the wrong pairing deliberately, since that is what yields a guaranteed window which cannot contain the base. The expectation is computed per fixture from what the capture itself states — a kallsyms column of a different width, or a release-keyed `/boot/config` naming another architecture — so a capture that states neither is counted and reported rather than silently passing |
