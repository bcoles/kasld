...
static void top_virt_text_base(struct estimate *e) {
   ...
   * [KASLR_TEXT_MIN_WIDE, KASLR_TEXT_MAX] — fixed per-arch by the kernel's
   * VA layout (unlike the physical base, this does not depend on DRAM
   * placement), so it is a sound and tighter honest top than the raw
   * mapping-region bounds KERNEL_TEXT_MIN/MAX.
   *
   * The _WIDE floor is the conservative variant of KASLR_TEXT_MIN — same
   * value where the arch's KASLR_TEXT_MIN does not bake in a configurable
   * Kconfig knob; *wider* on arches like x86_64 where KASLR_TEXT_MIN
   ...
   * real evidence). */
  top_interval(e, (unsigned long)KASLR_TEXT_MIN_WIDE,
...
static void top_page_offset(struct estimate *e) {
   ...
   * KERNEL_VAS_START/END span every PAGE_OFFSET the arch admits (true on
   ...
   * follow-ups. */
  top_interval(e, (unsigned long)KERNEL_VAS_START,
...
#define KASLD_QUANTITY_LIST(X)                                                 \
  ...
  X(Q_PHYS_TEXT_BASE, "phys_text_base", LK_INTERVAL, top_phys_text_base, NULL, \
    0)                                                                         \
  X(Q_PAGE_OFFSET, "page_offset", LK_INTERVAL, top_page_offset, NULL, 0)       \
  X(Q_VMALLOC_BASE, "vmalloc_base", LK_INTERVAL, top_kernel_vas_window, NULL,  \
    0)                                                                         \
  X(Q_VMEMMAP_BASE, "vmemmap_base", LK_INTERVAL, top_kernel_vas_window, NULL,  \
    0)                                                                         \
  X(Q_KASLR_ALIGN, "kaslr_align", LK_MAXALIGN, top_maxalign, NULL, 0)          \
