...
// page_offset sits below the L4 VAS floor. vmemmap_size = directmap_size_tb *
// 16 GiB rounded up to whole TiB (one 64-byte struct page per 4 KiB page).
//
...
// needs SF_MAX_PFN and is emitted only when present.
//
...
int rule_x86_64_vmemmap_base_bound(const struct evidence_set *ev,
  ...
  if (vmalloc->lo_binding == 0)
    return 0;
  unsigned long vmalloc_base_min = vmalloc->lo;

  unsigned long one_tb = 1ul << TB_SHIFT;
  unsigned long pud_size = 1ul << PUD_SHIFT;

  /* Paging mode from page_offset: L5 if its floor sits below the L4 VAS. */
  ...
  /* ---- Lower bound ---- */
  unsigned long lower = vmalloc_base_min + vmalloc_size_tb * one_tb + pud_size;
  if (lower > vmalloc_base_min && lower < CPU_ENTRY_AREA_BASE && n < out_max) {
