...
//
// The module_region_size version spread (128M/256M/2G) is folded into
// ARM64_MODULE_REGION_SIZE (2G, the v6.2+ default): the pin is inferred, so an
// older-kernel mismatch is overridden by any real leak.
//
...
int rule_arm64_text_base(const struct evidence_set *ev,
  ...
  unsigned long kimg = arm64_page_end_for(va_min) + ARM64_MODULE_REGION_SIZE;
  unsigned long ceiling = kimg + arm64_kaslr_offset_max(va_min);

  int n = 0;
  /* Narrow the union honest top to this VA_BITS_MIN's text band (KASLR on or
   * off): floor at KIMAGE_VADDR (admits the no-KASLR base too), ceiling at the
   * widest KASLR-window top. Inferred — a real leak overrides. */
  if (n < out_max) {
  ...
  /* No-KASLR: pin the base to the link-time default KIMAGE_VADDR(VA_BITS_MIN).
   * Capped to the disabled signal's confidence (and to inferred), so a real
   * text leak still wins. Window-containment: skip if the default falls outside
   * the current honest window (e.g. a real leak already excludes it). */
  uint32_t sig_id = 0;
  ...
  if (sig_id != 0 && n < out_max) {
    unsigned long v = kimg + (unsigned long)IMAGE_BASE_OFFSET;
