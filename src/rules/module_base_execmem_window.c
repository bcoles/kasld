...
// was set. Nothing observes the option directly, so the rule requires evidence
// that the IMAGE MOVED — a resolved Q_VIRT_IMAGE_BASE that excludes the
...
// MODULES_VADDR. Without that evidence the base could sit at the =n placement
// instead, and the window would be in the wrong place entirely; the rule stays
