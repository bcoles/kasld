...
//
// Rule: page_offset upper bound from the lowest leaked directmap address.
//
...
//
//   page_offset <= min(leaked DIRECTMAP virtual addresses)
//
...
//   page_offset >= V - (max_pfn*PAGE_SIZE - PHYS_OFFSET).
// max_pfn (SF_MAX_PFN, /proc/zoneinfo) is the kernel's own direct-map extent —
// the SOUND, tight span (not MemTotal, which undercounts physical address space
// by the reserved/firmware regions and would make the bound too high). With it,
// a directmap leak pins the randomized direct-map base to within max_pfn pages
// (~RAM/1GiB PUD-aligned candidates). Emits nothing without SF_MAX_PFN —
...
// page_offset_base on PUD-aligned boundaries (arch/x86/mm/kaslr.c:166
...
int rule_directmap_page_offset_bounds(const struct evidence_set *ev,
  ...
  for (int i = 0; i < ev->n_obs; i++) {
    ...
    if (!o->valid)
      continue;
    if (o->value_kind == OBS_SCALAR && o->scalar_fact == SF_MAX_PFN) {
   ...
   * the kernel's KASLR code places page_offset_base on. Zero on arches
  ...
  int n = 0;
  /* Upper bound: page_offset <= lowest directmap leak, aligned DOWN to PUD. */
  ...
  /* Lower bound: page_offset >= V - (max_pfn*PAGE_SIZE - PHYS_OFFSET),
   * aligned UP to PUD. */
