...
int main(void) {
  ...
  kasld_result(KASLD_ADDR_PHYS, KASLD_SECTION_DRAM, r.lo, "dmesg_swiotlb:lo");

  if (r.hi && r.hi != r.lo) {
    printf("SWIOTLB end:   0x%016lx\n", r.hi);
    kasld_result(KASLD_ADDR_PHYS, KASLD_SECTION_DRAM, r.hi, "dmesg_swiotlb:hi");
  }
