...
static struct observation *obs_by_id(struct evidence_set *ev, uint32_t id) {
  for (int i = 0; i < ev->n_obs; i++)
    if (ev->obs[i].id == id)
      return &ev->obs[i];
  TH_CHECK(0 && "obs_by_id: id not found");
...
static void test_add_assigns_monotonic_ids(void) {
  ...
  TH_CHECK(ia == 1 && ib == 2); /* monotonic from 1 */
  TH_CHECK(ev.n_obs == 2);
  /* Source fields preserved. */
  TH_CHECK(obs_by_id(&ev, ia)->region == REGION_RAM);
  TH_CHECK(obs_by_id(&ev, ib)->lo == 0xffffffff81000000ul);
}
...
static void test_resolve_no_verdicts_all_active(void) {
  ...
  TH_CHECK(evidence_active(o));
  TH_CHECK(o->eff_region == REGION_RAM && o->eff_type == KASLD_TYPE_PHYS);
}
...
static void test_invalidate(void) {
  ...
  evidence_resolve(&ev);
  TH_CHECK(evidence_active(obs_by_id(&ev, ia)));
  TH_CHECK(!evidence_active(obs_by_id(&ev, ib)));
}
...
static void test_resolve_idempotent_and_reversible(void) {
  ...
  evidence_resolve(&ev);
  TH_CHECK(!evidence_active(obs_by_id(&ev, id)));
...
static void test_scalar_observation(void) {
  ...
  evidence_resolve(&ev);
  struct observation *o = obs_by_id(&ev, id);
  TH_CHECK(o->value_kind == OBS_SCALAR);
  TH_CHECK(o->scalar_fact == SF_PHYS_MEMTOTAL);
  TH_CHECK(o->scalar_value == 12884901888ul);
  TH_CHECK(evidence_active(o));
...
static void test_image_size_accessors(void) {
  ...
  TH_CHECK(evidence_image_size_min(&ev, NULL, NULL) == 0);
  TH_CHECK(evidence_image_size_max(&ev, NULL, NULL) == 0);
  TH_CHECK(evidence_image_size_min_or_floor(&ev) == KASLD_MIN_IMAGE_SIZE);
  ...
  evidence_add(&ev, &lo2);
  evidence_resolve(&ev);
  TH_CHECK(evidence_image_size_min(&ev, NULL, NULL) == 0x3000000ul);
  /* A MIN-only fact never feeds the MAX (floor) accessor. */
  TH_CHECK(evidence_image_size_max(&ev, NULL, NULL) == 0);
  TH_CHECK(evidence_image_size_min_or_floor(&ev) == 0x3000000ul);
  ...
  evidence_add(&ev, &hi2);
  evidence_resolve(&ev);
  TH_CHECK(evidence_image_size_max(&ev, NULL, NULL) == 0x3000000ul);
  ...
  evidence_resolve(&ev);
  TH_CHECK(evidence_image_size_min(&ev, NULL, NULL) == 0x100000ul);
  TH_CHECK(evidence_image_size_min_or_floor(&ev) == KASLD_MIN_IMAGE_SIZE);
}
