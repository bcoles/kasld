...
static void test_replay_uses_only_the_file_signal(void) {
  ...
  if (sizeof(kasld_addr_t) < 8) {
    TH_CHECK(w.verdict == KASLD_WIDTH_MISMATCH);
    TH_CHECK(w.signal == KASLD_WIDTH_SIGNAL_KALLSYMS);
    ...
    TH_CHECK(w.verdict == KASLD_WIDTH_OK);
  }
}
...
static void test_no_signal_is_not_a_mismatch(void) {
  ...
  struct kasld_width_check w = kasld_check_target_width(KASLD_FACTS_CAPTURE);
  TH_CHECK(w.verdict == KASLD_WIDTH_OK);
  TH_CHECK(w.signal == KASLD_WIDTH_SIGNAL_NONE);
}
...
static void test_64bit_build_is_inert(void) {
  ...
  TH_CHECK(kasld_check_target_width(KASLD_FACTS_LIVE).verdict ==
           KASLD_WIDTH_OK);
  TH_CHECK(kasld_check_target_width(KASLD_FACTS_CAPTURE).verdict ==
           KASLD_WIDTH_OK);
}
...
static void test_refusal_document_shape(void) {
  ...
  TH_CHECK(kasld_width_signal_name(w.signal) != NULL);
  TH_CHECK(strstr(kasld_width_signal_name(w.signal), "boundary") != NULL);
  w.signal = KASLD_WIDTH_SIGNAL_KALLSYMS;
  TH_CHECK(strstr(kasld_width_signal_name(w.signal), "kallsyms") != NULL);
  /* An unset signal still names something printable rather than NULL. */
  w.signal = KASLD_WIDTH_SIGNAL_NONE;
  TH_CHECK(strcmp(kasld_width_signal_name(w.signal), "none") == 0);
}
