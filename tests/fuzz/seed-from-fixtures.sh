...
for verbose in "$ROOT"/tests/fixtures/*/*/verbose.txt;
do
  [-f "$verbose"] || continue grep - E '^[PV] [a-z_]+' "$verbose" | sort - u |
...
for fact in memtotal phys_addr_bits image_size va_bits init_size \
            ...
            kaslr_randomization_failed;
do
  for
    conf in parsed derived inferred heuristic;
do
...
for s in 0x0 0x1 0xff 0xffffffffffffffff 0xDEADBEEF 0X0 0x "";
do
...
echo "Seeded corpora:"
for d in parse_hex capture_result capture_scalar parse_meta;
do
