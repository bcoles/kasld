...
static void test_interval_meet_bounds(void) {
  struct estimate e;
  quantities[Q_VIRT_TEXT_BASE].init_top(&e);
  unsigned long top_lo = e.lo, top_hi = e.hi;

  struct constraint up = mk(Q_VIRT_TEXT_BASE, C_UPPER_BOUND,
                            top_lo + 0x10000000ul, CONF_PARSED, 1);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &up);
  assert(e.hi == top_lo + 0x10000000ul);
  assert(e.hi_binding == 1);
  assert(e.lo == top_lo); /* unchanged */
  assert(!estimate_is_bottom(&e, &quantities[Q_VIRT_TEXT_BASE]));

  struct constraint lo =
      mk(Q_VIRT_TEXT_BASE, C_LOWER_BOUND, top_lo + 0x1000000ul, CONF_PARSED, 2);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &lo);
  assert(e.lo == top_lo + 0x1000000ul);
  assert(e.lo_binding == 2);

  /* A looser upper bound does not widen. */
  struct constraint loose =
      mk(Q_VIRT_TEXT_BASE, C_UPPER_BOUND, top_hi, CONF_PARSED, 3);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &loose);
...
static void test_interval_meet_equals_and_bottom(void) {
  ...
  struct constraint eq =
      mk(Q_VIRT_TEXT_BASE, C_EQUALS, base + 0x2000000ul, CONF_DERIVED, 1);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &eq);
  assert(e.lo == base + 0x2000000ul && e.hi == base + 0x2000000ul);

  /* Contradicting equals → bottom. */
  struct estimate e2;
  quantities[Q_VIRT_TEXT_BASE].init_top(&e2);
  struct constraint a =
      mk(Q_VIRT_TEXT_BASE, C_UPPER_BOUND, base + 0x1000000ul, CONF_PARSED, 1);
  struct constraint b =
      mk(Q_VIRT_TEXT_BASE, C_LOWER_BOUND, base + 0x2000000ul, CONF_PARSED, 2);
  estimate_meet(&e2, &quantities[Q_VIRT_TEXT_BASE], &a);
  estimate_meet(&e2, &quantities[Q_VIRT_TEXT_BASE], &b);
  assert(estimate_is_bottom(&e2, &quantities[Q_VIRT_TEXT_BASE]));
}
...
static void test_resolve_stronger_wins(void) {
  ...
  cs[0] =
      mk(Q_VIRT_TEXT_BASE, C_UPPER_BOUND, base + 0x1000000ul, CONF_PARSED, 10);
  cs[1] =
      mk(Q_VIRT_TEXT_BASE, C_LOWER_BOUND, base + 0x2000000ul, CONF_TIMING, 11);

  struct resolve_result r;
  estimate_resolve(Q_VIRT_TEXT_BASE, CONF_BRUTE, cs, 2, &r);
...
static void test_resolve_priority_flips_with_confidence(void) {
  ...
  struct constraint cs[2];
  cs[0] =
      mk(Q_VIRT_TEXT_BASE, C_UPPER_BOUND, base + 0x1000000ul, CONF_TIMING, 10);
  cs[1] =
      mk(Q_VIRT_TEXT_BASE, C_LOWER_BOUND, base + 0x2000000ul, CONF_PARSED, 11);
...
static void test_resolve_deterministic_and_no_conflict(void) {
  ...
  struct constraint cs[2];
  cs[0] =
      mk(Q_VIRT_TEXT_BASE, C_UPPER_BOUND, base + 0x4000000ul, CONF_PARSED, 1);
  cs[1] =
      mk(Q_VIRT_TEXT_BASE, C_UPPER_BOUND, base + 0x2000000ul, CONF_DERIVED, 2);
...
static void test_estimate_at_trust_floor(void) {
  ...
  struct constraint cs[2];
  cs[0] = mk(Q_VIRT_TEXT_BASE, C_UPPER_BOUND, base + 0x4000000ul, CONF_PARSED,
             1); /* trusted, looser */
  cs[1] = mk(Q_VIRT_TEXT_BASE, C_UPPER_BOUND, base + 0x1000000ul, CONF_TIMING,
             2); /* shaky, tighter */

  struct resolve_result all, trusted;
  estimate_resolve(Q_VIRT_TEXT_BASE, CONF_BRUTE, cs, 2, &all);
  estimate_resolve(Q_VIRT_TEXT_BASE, CONF_PARSED, cs, 2, &trusted);
...
static void test_quantity_ranges_interval_plain(void) {
  ...
  quantities[Q_VIRT_TEXT_BASE].init_top(&e);
  struct range out[8];
  int n = quantity_ranges(Q_VIRT_TEXT_BASE, &e, NULL, 0, out, 8);
  assert(n == 1);
...
static void test_quantity_ranges_interval_with_interior_hole(void) {
  ...
  cs[0] = mk(Q_VIRT_TEXT_BASE, C_EXCLUDE, base + 0x1000000ul, CONF_DERIVED, 1);
  cs[0].value2 = base + 0x1fffffful; /* hole [base+16M, base+32M-1] */

  struct range out[8];
  int n = quantity_ranges(Q_VIRT_TEXT_BASE, &e, cs, 1, out, 8);
  assert(n == 2);
...
static void test_quantity_slots_hole_aware(void) {
  ...
  unsigned long full = quantity_slots(Q_VIRT_TEXT_BASE, &e, NULL, 0, align);
  ...
  cs[0] = mk(Q_VIRT_TEXT_BASE, C_EXCLUDE, base + 0x1000000ul, CONF_DERIVED, 1);
  cs[0].value2 = base + 0x1fffffful; /* hole [base+16M, base+32M-1] */
  unsigned long holed = quantity_slots(Q_VIRT_TEXT_BASE, &e, cs, 1, align);
  assert(holed < full);

  /* align 0 is defined as 0 slots (no division). */
  assert(quantity_slots(Q_VIRT_TEXT_BASE, &e, NULL, 0, 0) == 0);
}
...
static void test_honest_tops_admit_known_values(void) {
  ...
  assert(interval_admits(Q_PHYS_TEXT_BASE, (unsigned long)PHYSICAL_START));
  assert(interval_admits(Q_PHYS_TEXT_BASE, (unsigned long)KASLR_PHYS_MIN));
  assert(interval_admits(Q_PHYS_TEXT_BASE, (unsigned long)KASLR_PHYS_MIN_WIDE));
  assert(!interval_admits(Q_PHYS_TEXT_BASE,
                          (unsigned long)KASLR_PHYS_MIN_WIDE - 1ul));
  assert(interval_admits(Q_PHYS_TEXT_BASE, 0x4000000000ul)); /* 256 GiB */
  assert(!interval_admits(Q_PHYS_TEXT_BASE, PHYS_ADDR_TOP + 1ul));
...
static void test_stride_first_constraint_sets_annotation(void) {
  ...
  quantities[Q_VIRT_TEXT_BASE].init_top(&e);
  struct constraint c =
      mk_stride(Q_VIRT_TEXT_BASE, 0x1234ul, 0x100000ul /* 1 MiB */, 1);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &c);
...
static void test_stride_same_modulus_agreeing_residues(void) {
  struct estimate e;
  quantities[Q_VIRT_TEXT_BASE].init_top(&e);
  struct constraint c1 = mk_stride(Q_VIRT_TEXT_BASE, 0x1000ul, 0x10000ul, 1);
  struct constraint c2 = mk_stride(Q_VIRT_TEXT_BASE, 0x11000ul, 0x10000ul, 2);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &c1);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &c2);
...
static void test_stride_same_modulus_disagreeing_residues_bottom(void) {
  ...
  quantities[Q_VIRT_TEXT_BASE].init_top(&e);
  struct constraint c1 = mk_stride(Q_VIRT_TEXT_BASE, 0x1000ul, 0x10000ul, 1);
  struct constraint c2 = mk_stride(Q_VIRT_TEXT_BASE, 0x2000ul, 0x10000ul, 2);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &c1);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &c2);
  assert(estimate_is_bottom(&e, &quantities[Q_VIRT_TEXT_BASE]));
}
...
static void test_stride_crt_combines_to_lcm(void) {
  ...
  struct constraint c1 = mk_stride(Q_VIRT_TEXT_BASE, 1ul, 6ul, 1);
  struct constraint c2 = mk_stride(Q_VIRT_TEXT_BASE, 4ul, 9ul, 2);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &c1);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &c2);
...
static void test_stride_crt_inconsistent_bottom(void) {
  ...
  quantities[Q_VIRT_TEXT_BASE].init_top(&e);
  struct constraint c1 = mk_stride(Q_VIRT_TEXT_BASE, 1ul, 6ul, 1);
  struct constraint c2 = mk_stride(Q_VIRT_TEXT_BASE, 5ul, 9ul, 2);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &c1);
  estimate_meet(&e, &quantities[Q_VIRT_TEXT_BASE], &c2);
  assert(estimate_is_bottom(&e, &quantities[Q_VIRT_TEXT_BASE]));
}
...
static void test_stride_no_intersection_with_interval_bottom(void) {
  ...
  struct constraint c = mk_stride(Q_VIRT_TEXT_BASE, 7ul, 16ul, 1);
...
static void test_quantity_slots_with_stride(void) {
  struct constraint cs[1];
  cs[0] = mk_stride(Q_VIRT_TEXT_BASE, 0x200000ul, 0x100000ul, 1);
  struct estimate e;
  quantities[Q_VIRT_TEXT_BASE].init_top(&e);
  ...
  unsigned long slots =
      quantity_slots(Q_VIRT_TEXT_BASE, &e, cs, 1, 0x4000ul /* 16 KiB */);
